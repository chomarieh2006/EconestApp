...
export default function AppliancePopup({
  userId,                    // pass your logged‐in user’s ID here
  userUsage = 600,
  averageUsage = 500,
  ...
  // // 1) fetch current appliance on mount
  // useEffect(() => {
  //   (async () => {
  //     try {
  ...
  //   if (!form.name || !form.usage) return alert('Fill all fields!');
  //   const newAppliance = {
  //     name: form.name,
  //     usage: parseInt(form.usage, 10),
  //     image: form.image,            // in prod you'd upload to Storage & save URL
  //   };
  //   try {
  //     await setDoc(
  //       doc(db, 'users', userId),
        ...
        {/* ——— Current Appliance Card ——— */}
          ...
          <View style={styles.recCard}>
            ...
            <View style={styles.recInfo}>
              <Text style={styles.recName}>{'YOUR FRIDGE'}</Text>
              <Text style={styles.recUsage}>
                {'500'} Kilowatt‑Hours per year
              </Text>
            </View>
            <TouchableOpacity
...
const styles = StyleSheet.create({
  overlay: {
    ...
    backgroundColor: 'rgba(0, 0, 0, 0.14)', // or whatever backdrop you like
